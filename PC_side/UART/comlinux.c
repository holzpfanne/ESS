...
int main(int Parm_Count, char *Parms[])
{
   ...
   char buf[255] = {0};           //buffer for where data is put
   ...
   {                                                          //run the program
      ...
      char send_buffer[16] = {0}, read_buffer[256] = {0};
      ...
      while (STOP == FALSE)
      {
         ...
         if (status == 1) //if a key was hit
         {
            ...
            case '\n':
               ...
               write(fd, &send_buffer, strlen(send_buffer)); //write message to the port
            ...
            default:
               ...
               //                  fputs(message,output);
               send_buffer[strlen(send_buffer)] = Key;
               break;
            } //end of switch key
         ...
         if (wait_flag == FALSE) //if input is available
         {
            ...
            if (res > 0)
            {
               ...
               char dead[16];
               if (sscanf(read_buffer, "#a,%s\n", dead) == 1)
               {
                  //received message was in correct form
                  write(fd, "ACK\n", 4);
               }
               else if (strncmp(read_buffer, "ACK\n", 4) == 0)
               {
                  //message was send correct
               }
               
               if (strchr(read_buffer, '\n') != NULL)
               {
                  //write crage return to output to set cursor to the letf
                  fputc((int)'\r', output);

                  //delte string part befor \n and also \n
                  int clear = (strchr(read_buffer, '\n') - read_buffer) + 1;
                  memcpy(read_buffer, strchr(read_buffer, '\n') + 1, 256 - clear);
                  memset(strchr(read_buffer, 0), 0, 256 - clear);
               }
